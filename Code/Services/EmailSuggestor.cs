...
{
    public static class EmailSuggestor
    {
        public static EmailSuggestionResult Suggest(HashSet<string> haystack, string needle)
        {
            var result = new EmailSuggestionResult {Score = 100};

            var sift = new Sift4();

            foreach (var source in haystack)
            {
                var distance = sift.GeneralDistance(needle, source, 5);
    ...
    public class Sift4
    {
        ...
        public Sift4(Options options)
        {
            ...
            {
                options.Tokenizer = s => s?.ToCharArray().Select(c => c.ToString()).ToArray() ?? new string[0];
            }
        ...
        public double GeneralDistance(string s1, string s2, int maxOffset)
        {
            ...
            while (c1 < l1 && c2 < l2)
            {
                ...
                {
                    var temporaryDistance = _options.LocalLengthEvaluator(Math.Max(c1, c2)) -
                                            _options.TranspositionsEvaluator(lcss, trans);
            ...
            lcss += _options.LocalLengthEvaluator(localCs);
            return Math.Round(
                _options.LocalLengthEvaluator(Math.Max(l1, l2)) - _options.TranspositionsEvaluator(lcss, trans),
                MidpointRounding.AwayFromZero); //apply transposition cost to the final result
        }

        /// <summary>
        ///     Static distance algorithm working on strings, computing transpositions as well as stopping when maxDistance was
        ///     reached.
        /// </summary>
        /// <param name="s1"></param>
        /// <param name="s2"></param>
        ...
        public class Options
        {
            ...
            /// <summary>
            ///     The function that determines if two tokens are matching (similar to characters being the same in the simple
            ///     algorithm)
            /// </summary>
            public Func<string, string, bool> TokenMatcher { get; set; }

            /// <summary>
            ///     The function that determines the value of a match of two tokens (the equivalent of adding 1 to the lcss when two
            ///     characters match)
            ///     This assumes that the TokenMatcher function is a lot less expensive than this evaluator. If that is not the case,
            ///     you can optimize the speed of the algorithm by using only the matching evaluator and then calculating if two tokens
            ///     match on the returned value.
            /// </summary>
