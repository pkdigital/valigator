...
namespace Valigator.Services
{
    ...
    {
        private const string ATOM_CHARACTERS = "!#$%&'*+-/=?^_`{|}~";
        ...
        {
            return c < 128 ? IsLetterOrDigit(c) || ATOM_CHARACTERS.IndexOf(c) != -1 : allowInternational;
        }
        ...
        ///     <para>
        ///         If <paramref name="allowTopLevelDomains" /> is <c>true</c>, then the validator will
        ///         allow addresses with top-level domains like <c>postmaster@dk</c>.
        ///     </para>
        ///     <para>
        ///         If <paramref name="allowInternational" /> is <c>true</c>, then the validator
        ...
        /// <param name="allowTopLevelDomains">
        ///     <c>true</c> if the validator should allow addresses at top-level domains; otherwise,
        ///     <c>false</c>.
        /// </param>
        /// <param name="allowInternational">
        ///     <c>true</c> if the validator should allow international characters; otherwise,
        ///     <c>false</c>.
        /// </param>
        /// <exception cref="System.ArgumentNullException">
        ///     <paramref name="email" /> is <c>null</c>.
        /// </exception>
        public static EmailParserResult Validate(string email, bool allowTopLevelDomains = false,
            bool allowInternational = false)
        {
            var result = new EmailParserResult();
            var index = 0;

            if (string.IsNullOrWhiteSpace(email))
            {
                return result.Invalid(ParserRule.AddressEmpty);
            }

            if (email.Length == 0)
            {
                return result.Invalid(ParserRule.AddressTooShort);
            }

            if (email.Length >= 255)
            {
                return result.Invalid(ParserRule.AddressTooLong);
            }
            ...
            if (email[index] == '"')
            {
                ...
                {
                    return result.Invalid(ParserRule.UnbalancedLiteral);
                }
            }
            else
            {
            ...
            // set the local part and domain in the return object
            result.LocalPart = email.Substring(0, index - 1).ToLower();
            result.Domain = email.Substring(index).ToLower();
            ...
            if (email[index] != '[')
            {
                ...
                return
                    index == email.Length ? result.Valid() : result.Invalid(ParserRule.UnbalancedLiteral);
            }
            ...
            {
                return result.Invalid(ParserRule.AddressLiteralTooShort);
            }
            ...
            if (ipv6.ToLowerInvariant() == "ipv6:")
            {
                ...
                {
                    return result.Invalid(ParserRule.InvalidIPv6Address);
                }
            }
            else
            {
                ...
                {
                    return result.Invalid(ParserRule.InvalidIPv4Address);
                }
            }
